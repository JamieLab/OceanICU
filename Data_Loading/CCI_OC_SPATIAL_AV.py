...
def load_sst(file):
    c = Dataset(file,'r')
    sst = np.transpose(np.array(c.variables['chlor_a'][0,:,:]))
...
def netcdf_create(file,sst,lat,lon):
    ...
    outp.code_used = 'CCI_OC_spatial_average.py'
    ...
    sst_o = outp.createVariable('chlor_a','f4',('lon','lat'),**copts) #Fill Value is set as this (can't make it the same as ESA for some reason?)
    sst_o[:] = sst
...
while ye <= en_ye:
    ...
    file_o = os.path.join(out_folder,str(ye),'ESACCI-OC-L3S-CHLOR_A-MERGED-1M_MONTHLY_'+str(ye)+numstr(mon)+'-fv6.0_1_deg.nc')
    print(file)
    if t == 0:
