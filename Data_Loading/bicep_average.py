...
def bicep_pic_log_average(loc,outloc,filename_struct='',start_yr=1990,end_yr=2023,res=1,netcdf_var = 'PIC',make_month_folder = True,log='',lag='',area_wei=False,
    gebco_file = False,gebco_out=False,land_mask=False):
    du.makefolder(outloc)
    # log,lag = du.reg_grid(lon=res,lat=res)
    res = np.abs(log[0] - log[1])
    ...
    while yr <= end_yr:
        ...
        if du.checkfileexist(file) and not du.checkfileexist(outfile):
            ...
            if area_wei:
                ...
                c.area_weighting = 'False'
            c.created_with = 'bicep_average.bicep_pic_log_average'
            c.close()
        mon = mon+1
        if mon == 13:
            yr = yr+1
