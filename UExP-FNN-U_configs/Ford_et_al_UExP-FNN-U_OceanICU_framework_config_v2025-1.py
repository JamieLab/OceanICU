...
if __name__ == '__main__':
    ...
    run_neural =False
    run_flux = True
    run_flux_testing = False
    plot_flux_carbonate = False
    geopan=True

    run_british_eez = False
    run_salinity_k = False
    run_bubble_flux = False
...
#         # # #
        ...
        interpolate_noaa('F:/Data/NOAA_ERSL/2025_download.txt',grid_lon = log,grid_lat = lag,out_dir = os.path.join(inps,'xco2atm'),start_yr=start_yr,end_yr = end_yr)
#         #
...
#         socat_file = 'E:/Data/_Datasets/SOCAT/v2025/SOCATv2025_ESACCIv3_biascorrected_Fordetal_Humpherys_daily.nc'
...
#         # cinp.extract_independent_test(data_file,'CCI_SST','F:/Data/RECCAP/RECCAP2_region_masks_all_v20221025_DJF.nc','reccap')
#         # cinp.plot_indpendent(data_file,model_save_loc,'CCI_SST','F:/Data/RECCAP/RECCAP2_region_masks_all_v20221025_DJF.nc','reccap')
#
    if run_neural:
        ...
        nnt.driver(data_file,fco2_sst = 'CCI_SST', prov = 'prov_smoothed',var = ['CCI_SST_analysed_sst','NOAA_ERSL_xCO2','CCI_SSS_sss_CMEMS_so','CMEMS_mlotst','CCI_SST_analysed_sst_anom','NOAA_ERSL_xCO2_anom','CCI_SSS_sss_CMEMS_so_anom','CMEMS_mlotst_anom'],
           ...
           tot_lut_val=20000,socat_sst=True)
    ...
    if run_flux:
        ...
        fl.montecarlo_flux_testing(model_save_loc,decor='fco2_net_decorrelation.csv',flux_var = 'flux_unc_fco2sw_net',start_yr =start_yr,end_yr=end_yr,fluxloc = os.path.join(model_save_loc,'flux_nocool_nosalt_night'),inp_file = os.path.join(model_save_loc,'output_nocool_nosalt_night.nc'),output_file='annual_flux_nocool_nosalt_night.csv')
        fl.montecarlo_flux_testing(model_save_loc,decor='fco2_decorrelation.csv',flux_var = 'flux_unc_fco2sw_para',start_yr =start_yr,end_yr=end_yr,fluxloc = os.path.join(model_save_loc,'flux_nocool_nosalt_night'),inp_file = os.path.join(model_save_loc,'output_nocool_nosalt_night.nc'),output_file='annual_flux_nocool_nosalt_night.csv')
        fl.montecarlo_flux_testing(model_save_loc,decor='fco2_decorrelation.csv',flux_var = 'flux_unc_fco2sw_val',start_yr =start_yr,end_yr=end_yr,fluxloc = os.path.join(model_save_loc,'flux_nocool_nosalt_night'),inp_file = os.path.join(model_save_loc,'output_nocool_nosalt_night.nc'),output_file='annual_flux_nocool_nosalt_night.csv')

    if run_flux_testing:
        # from Data_Loading.interpolate_noaa_ersl import interpolate_noaa
        # interpolate_noaa('F:/Data/NOAA_ERSL/2025_download_v2.txt',grid_lon = log,grid_lat = lag,out_dir = os.path.join(inps,'xco2atm_2'),start_yr=start_yr,end_yr = end_yr)
        # import construct_input_netcdf as cinp
        # vars = [['NOAA_ERSL_v2','xCO2',os.path.join(inps,'xco2atm_2','%Y','%Y_%m*.nc'),1]]
        # cinp.driver(data_file,vars,start_yr = start_yr,end_yr = end_yr,lon = log,lat = lag,append=True,fill_clim=False)
        import fluxengine_driver as fl
        print('Running flux calculations....')
        fluxengine_config = 'C:/Users/df391/OneDrive - University of Exeter/Post_Doc_ESA_Contract/OceanICU/fluxengine_config/fluxengine_config_night.conf'
        ...
        #      sal='CCI_SSS_sss_CMEMS_so',msl = 'ERA5_msl',xCO2 = 'NOAA_ERSL_xCO2',start_yr=start_yr,end_yr=end_yr, coare_out = os.path.join(inps,'coare'), tair = 'ERA5_t2m', dewair = 'ERA5_d2m',
        #      rs = 'ERA5_msdwswrf', rl = 'ERA5_msdwlwrf', zi = 'ERA5_blh',coolskin = 'COARE3.5')
        fl.fluxengine_run(model_save_loc,fluxengine_config,start_yr,end_yr,output_ov='flux_testing2')
        fl.flux_uncertainty_calc(model_save_loc,start_yr = start_yr,end_yr=end_yr,fco2_tot_unc = -1,k_perunc=0.2,unc_input_file=data_file,sst_unc='CCI_SST_analysed_sst_uncertainty',atm_unc=0.4,flux_loc = 'flux_testing2',override_output='output_flux_testing2.nc')
        fl.calc_annual_flux(model_save_loc,lon=log,lat=lag,start_yr=start_yr,end_yr=end_yr,flux_file = os.path.join(model_save_loc,'output_flux_testing2.nc'),save_file=os.path.join(model_save_loc,'annual_flux_testing2.csv'))
