...
def GetCustom(years,months,lons,lats,SSTdir, SSTtail, customsstvar,customsstlat,customsstlon):
    # print(customsstvar)
    # print(customsstlat)
    # print(customsstlon)
    return GetSST(years, months, lons, lats, SSTdir, SSTtail,dataname=customsstvar,lonname=customsstlon,latname=customsstlat)

def ReadSSTFile(filename,dataname='sst_skin_mean',lonname='lon',latname='lat'):
   """
   Daniel J. Ford - 03/2023
   Modified this function to be flexible in the data resolution used (i.e not hardcoded to 1deg)
   """
   ...
   with netCDF4.Dataset(filename,'r') as SST_file:
      d = numpy.squeeze(numpy.array(SST_file.variables[dataname][:]))
      lats = SST_file.variables[latname][:]
      #Making extended lon variable
      lon = SST_file.variables[lonname][:]
      lons = numpy.zeros((len(lon)+2))
      lons[1:-1] = lon
      lons[0] = lon[-1]
      lons[-1] = lon[0]

      #Extended data variables
      data = numpy.zeros((len(lats),len(lon)+2))
      ...
      try:
          if SST_file.variables[dataname].units in ['degrees C']:
            ...
            print('Assuming SST units are Kelvin')
