...
def bicep_pp_log_average(loc,outloc,start_yr=1990,end_yr=2023,res=1):
    ...
    while yr <= end_yr:
        ...
        file = os.path.join(loc,str(yr),'BICEP_NCEO_PP_ESA-OC-L3S-MERGED-1M_MONTHLY_9km_mapped_'+str(str(yr)) + du.numstr(mon)+'-fv4.2.nc')
        outfile = os.path.join(outloc,str(yr),str(yr)+'_'+du.numstr(mon)+'_BICEP_NCEO_PP_ESA-OC-L3S-MERGED-1M_MONTHLY_'+str(res)+'_deg.nc')
        print(file)
        print(outfile)
        if du.checkfileexist(file) and not du.checkfileexist(outfile):
            ...
            c = Dataset(file,'r')
            va_da = np.transpose(np.squeeze(np.array(c.variables['pp'][:])))
            ...
            du.netcdf_create_basic(outfile,va_da_out,'pp',lag,log)
        mon = mon+1
        if mon == 13:
            yr = yr+1
            mon=1
