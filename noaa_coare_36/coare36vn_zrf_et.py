...
# input us = 0*u here;
    ...
    ice = np.zeros(N)
    iice = np.squeeze(np.array(np.where(ts < Tf-30))) # DJF modified to remove ice check, and had to do a np.squeeze as this had 2 dimensions when the inputs only have 1 dimension.
    ice[iice] = 1
    jcool[iice] = 0
    zos = 0.0005
    #***********  set constants ***********************************************
    ...
    for i in np.arange(1,nits+1).reshape(-1):
        ...
        #ik = np.array(np.where(rr[iice] > np.logical_and(0.135,rr[iice]) <= 2.5)) #DJF: Had to fix these as kept crashing...
        ...
        #ik = np.array(np.where(rr[iice] > np.logical_and(2.5,rr[iice]) <= 1000)) #DJF: Had to fix these as kept crashing...
