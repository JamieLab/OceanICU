...
if __name__ == '__main__':
    ...
    model_save_loc = 'F:/OceanCarbon4Climate/GCB2026'
...
#         cmems_average('F:/Data/CMEMS/SSS/MONTHLY',os.path.join(inps,'SSS'),start_yr=start_yr,end_yr=end_yr,log=log,lag=lag,variable='so')
#         cmems_average('F:/Data/CMEMS/MLD/MONTHLY',os.path.join(inps,'MLD'),start_yr=start_yr,end_yr=end_yr,log=log,lag=lag,variable='mlotst',log_av = True)
#         #
#         import Data_Loading.cci_sstv2 as cci2
#         cci2.cci_monthly_av('F:/Data/SST-CCI/v301',start_yr=start_yr,end_yr=end_yr,v3 = True)
#         cci2.cci_sst_spatial_average(data='F:/Data/SST-CCI/V301/monthly',out_loc=os.path.join(inps,'SST'),start_yr=start_yr,end_yr=end_yr,log=log,lag=lag,bia=0.04,v3 = True,monthly=True,area_wei=True)
# #         # # #
#         from Data_Loading.interpolate_noaa_ersl import interpolate_noaa
#         interpolate_noaa('F:/Data/NOAA_ERSL/2025_download_v2.txt',grid_lon = log,grid_lat = lag,out_dir = os.path.join(inps,'xco2atm'),start_yr=start_yr,end_yr = end_yr)
# #         #
#         from Data_Loading.ERA5_data_download import era5_average
#         era5_average(loc = "F:/Data/ERA5/MONTHLY/DATA", outloc=os.path.join(inps,'msl'),log=log,lag=lag,var='msl',start_yr = start_yr,end_yr =end_yr)
#         era5_average(loc = "F:/Data/ERA5/MONTHLY/DATA", outloc=os.path.join(inps,'blh'),log=log,lag=lag,var='blh',start_yr = start_yr,end_yr =end_yr)
#         era5_average(loc = "F:/Data/ERA5/MONTHLY/DATA", outloc=os.path.join(inps,'d2m'),log=log,lag=lag,var='d2m',start_yr = start_yr,end_yr =end_yr)
#         era5_average(loc = "F:/Data/ERA5/MONTHLY/DATA", outloc=os.path.join(inps,'t2m'),log=log,lag=lag,var='t2m',start_yr = start_yr,end_yr =end_yr)
#         era5_average(loc = "F:/Data/ERA5/MONTHLY/DATA", outloc=os.path.join(inps,'msdwlwrf'),log=log,lag=lag,var='msdwlwrf',start_yr = start_yr,end_yr =end_yr)
#         era5_average(loc = "F:/Data/ERA5/MONTHLY/DATA", outloc=os.path.join(inps,'msdwswrf'),log=log,lag=lag,var='msdwswrf',start_yr = start_yr,end_yr =end_yr)
# # #
#         era5_average(loc = "F:/Data/ERA5/DAILY/monthly", outloc=os.path.join(inps,'ws'),log=log,lag=lag,var='ws',start_yr = start_yr,end_yr =end_yr,land_mask=True,gebco_file='F:/Data/Bathymetry/GEBCO_2023.nc',gebco_out='F:/Data/Bathymetry')
#         era5_average(loc = "F:/Data/ERA5/DAILY/monthly", outloc=os.path.join(inps,'ws2'),log=log,lag=lag,var='ws2',start_yr = start_yr,end_yr =end_yr,land_mask=True,gebco_file='F:/Data/Bathymetry/GEBCO_2023.nc',gebco_out='F:/Data/Bathymetry')
# # # #         #
#         import Data_Loading.gebco_resample as ge
#         ge.gebco_resample('F:/Data/Bathymetry/GEBCO_2023.nc',log,lag,save_loc = os.path.join(inps,'bath_GEBCO.nc'),save_loc_fluxengine = os.path.join(inps,'fluxengine_bath_GEBCO.nc'))
# # # #         # #
...
#         OSISAF_spatial_average(data='F:/Data/OSISAF/monthly',out_loc=os.path.join(inps,'OSISAF'),start_yr=start_yr,end_yr=end_yr,log=log,lag=lag,hemi = 'NH')
#         OSISAF_spatial_average(data='F:/Data/OSISAF/monthly',out_loc=os.path.join(inps,'OSISAF'),start_yr=start_yr,end_yr=end_yr,log=log,lag=lag,hemi = 'SH')
...
#         cc.ccmp_temporal_average('F:/Data/CCMP/v3.1',v=3.1,var='ws',start_yr=start_yr,end_yr=end_yr)
#         cc.ccmp_average('F:/Data/CCMP/v3.1/monthly',outloc=os.path.join(inps,'ccmpv3.1'),start_yr=start_yr,end_yr=end_yr,log=log,lag=lag,v =3.1,var='ws',land_mask=True,gebco_file='F:/Data/Bathymetry/GEBCO_2023.nc',gebco_out='F:/Data/Bathymetry')
# # #
#         import Data_Loading.cci_sss as ccisss
#         ccisss.cci_sss_spatial_average(data='E:/Data/SSS-CCI/v5.5/%Y/ESACCI-SEASURFACESALINITY-L4-SSS-GLOBAL-MERGED_OI_Monthly_CENTRED_15Day_0.25deg-%Y%m15-fv5.5.nc',start_yr=start_yr,end_yr=end_yr,out_loc = os.path.join(inps,'CCISSS'),log=log,lag=lag,area_wei=True)
...
#         #Vars should have each entry as [Extra_Name, netcdf_variable_name,data_location,produce_anomaly]
        ...
        ,['OC-CCI','chl',os.path.join(inps,'chla','%Y','%Y_%m_*.nc'),1]
        ]
        import construct_input_netcdf as cinp
...
# # #
        ...
        som.som_feed_forward_probability(model_save_loc,data_file,['Takahashi_taka','CCI_SST_analysed_sst','CCI_SSS_sss_CMEMS_so','CMEMS_mlotst','OC-CCI_chl'],unc = [20,0.45,0.3,0.1,0.5],data_file_out=os.path.join(model_save_loc,'inputs','som_prob.nc'),ens=400)
...
#
    if run_neural:
        ...
        nnt.driver(data_file,fco2_sst = 'CCI_SST', prov = 'prov_smoothed',var = ['CCI_SST_analysed_sst','NOAA_ERSL_xCO2','CCI_SSS_sss_CMEMS_so','CMEMS_mlotst','OC-CCI_chl','CCI_SST_analysed_sst_anom','NOAA_ERSL_xCO2_anom','CCI_SSS_sss_CMEMS_so_anom','CMEMS_mlotst_anom','OC-CCI_chl_anom'],
           model_save_loc = model_save_loc +'/',unc =[0.35,0.4,0.2,0.1,0.5,0.35,0.4,0.2,0.1,0.5],bath = 'GEBCO_elevation',bath_cutoff = None,fco2_cutoff_low = 50,fco2_cutoff_high = 750,sea_ice = None,
           tot_lut_val=20000,socat_sst=True,prob_ensemble='prov_ensemble')
        ...
        #    tot_lut_val=6000,socat_sst=True,run_network_training=False)
    if run_flux:
        ...
        #      sal='CCI_SSS_sss_CMEMS_so',msl = 'ERA5_msl',xCO2 = 'NOAA_ERSL_xCO2',start_yr=start_yr,end_yr=end_yr, coare_out = os.path.join(inps,'coare'), tair = 'ERA5_t2m', dewair = 'ERA5_d2m',
        #      rs = 'ERA5_msdwswrf', rl = 'ERA5_msdwlwrf', zi = 'ERA5_blh',coolskin = 'COARE3.6')
        ...
        # fl.variogram_evaluation(model_save_loc,output_file='wind_decorrelation',input_array=['CCMP_ws','ERA5_ws'],input_datafile=[data_file,data_file],start_yr =start_yr,end_yr=end_yr)
        # #
        ...
        # fl.plot_relative_contribution(model_save_loc)
        ...
        # fl.fluxengine_run(model_save_loc,fluxengine_config,start_yr,end_yr,output_ov='flux_nocool_nosalt_night')
        # fl.flux_uncertainty_calc(model_save_loc,start_yr = start_yr,end_yr=end_yr,fco2_tot_unc = -1,k_perunc=0.2,unc_input_file=data_file,sst_unc='CCI_SST_analysed_sst_uncertainty',atm_unc=0.4,flux_loc = 'flux_nocool_nosalt_night',override_output='output_nocool_nosalt_night.nc')
        # fl.calc_annual_flux(model_save_loc,lon=log,lat=lag,start_yr=start_yr,end_yr=end_yr,flux_file = os.path.join(model_save_loc,'output_nocool_nosalt_night.nc'),save_file=os.path.join(model_save_loc,'annual_flux_nocool_nosalt_night.csv'))
    ...
    model_save_loc2 = os.path.join(model_save_loc,'ALKALINITY')
    GLODAP_file = 'F:/Data/_DataSets/GLODAP/GLODAPv2.2023_Merged_Master_File_SNAPOCO2appended_sharkwebappended_global_surface_v2.csv'
    ...
    if create_inp:
        ...
        co2sys.load_DIVA_glodap('F:/Data/_DataSets/GLODAP/window_data_from_GLODAPv2.2023_Merged_Master_File_SNAPOCO2appended_sharkwebappended_global_surface_v2.txt',delimiter='\t',out_file = os.path.join(model_save_loc2,'inputs','ta_interpolated.nc'),log=log,lag=lag,var_name='ta',inp_name='Estimated G2talk @ G2expocode=first')
        vars = [
        ...
        som.som_feed_forward_probability(model_save_loc2,data_file,['insitu_ta','CCI_SSS_sss_CMEMS_so','CCI_SST_analysed_sst'],unc = [20,0.2,0.35],data_file_out=os.path.join(model_save_loc2,'inputs','som_prob_ta.nc'),ens=400,normalise=True,o_var='TA_prov',m=4,box = [5,3])
        som.merge_provinces(os.path.join(model_save_loc2,'inputs','som_prob_ta.nc'),data_file,'TA_prov_smoothed','TA_prov_ensemble',out_prov = 'TA_prov_smoothed',out_prob='TA_prov_ensemble',province_dim_name='TA_province')
    ...
    if run_neural:
        ...
        nnt.daily_neural_driver(os.path.join(model_save_loc2,'inputs','glodap.csv'),fco2_sst = 'G2talk', prov = 'TA_prov_smoothed',var = ['G2temperature','G2salinity','p_an','i_an'],sep=',',
           ...
           year_v = 'G2year',mon_v ='G2month',day_v = 'G2day',prob_ensemble='TA_prov_ensemble')
    ...
    if run_pyco2:
        ...
        # co2sys.plot_pyCO2sys_out_flux(os.path.join(model_save_loc,'output.nc'),model_save_loc,geopan=geopan)
    ...
    if final_package:
        import global_carbon_budget.PlanetaryPackage as PP
        fluxloc = os.path.join(model_save_loc,'flux')
        version = '2026-pre1'
        output_file = os.path.join(model_save_loc,'Fordetal_UExP-FNN-U_surface-carbonate-system_v'+version+'.nc')
        PP.PlantaryHealthCheckPackage(model_save_loc,fluxloc,output_file,start_yr,end_yr,log,lag,version = version,copts={'zlib':True})

    uncertainty_aragonite_testing = True
    if uncertainty_aragonite_testing:
        import pyCO2sys_func as co2sys
        co2sys.montecarlo_mean_testing(model_save_loc,decor='fco2_decorrelation.csv',flux_var = 'saturation_aragonite_tot_unc',flux_variable='saturation_aragonite',start_yr =start_yr,end_yr=end_yr,output_file='aragonite_saturation_annual_mean.csv')
