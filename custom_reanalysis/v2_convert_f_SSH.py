...
def DoConversion(inputfile, columnInfo, startyr=cldefaults['start'],endyr=cldefaults['end'],prefix=cldefaults['prefix'],
                  ...
                  removeduplicates=not cldefaults['keepduplicates'],customsst=False, customsstvar = None,customsstlat=None,customsstlon=None):
   """
   ...
   for year_range in year_ranges_to_process:
      ...
      number_of_data_points,duplicates=ConvertYears(data,year_range,sstdir,ssttail,prefix,outputdir,
                                                ...
                                                useaatsr=useaatsr,usereynolds=usereynolds,customsst=customsst, customsstvar=customsstvar,
                                                customsstlat=customsstlat,customsstlon=customsstlon)
...
def ConvertYears(data,year_range,sstdir,ssttail,prefix,outputdir,extrapolatetoyear,version,
   ...
   if useaatsr and not usereynolds and not customsst:
      ...
      Tcls += 0.17
   elif usereynolds and not useaatsr and not customsst:
      ...
      """
      Daniel J. Ford
      Added ability to specify your own sst product, with laittude, longitude and sst variables defined.
      """
   elif customsst:
      Tcls = get_sst.GetCustom(data_subset['year'], data_subset['month'], data_subset['longitude'],
                              data_subset['latitude'],sstdir, ssttail,customsstvar,customsstlat,customsstlon)
   else:
