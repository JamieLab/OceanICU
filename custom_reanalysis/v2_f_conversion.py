...
def v2_f_conversion_wrap(jds,data_array,Tcls,Peq_cls,extrapolatetoyear=None):
   ...
   if jd is None:
      ...
      result=np.recarray((jd.size,),dtype=[('jd',np.float64),
                                           ('yr',np.int32),
                                           ('mon',np.int32),
                                           ('day', np.int32),
                                           ('hh', np.int32),
...
#      print "R:", R; # cm^3 atm/(mol K)
      ...
      """
      Daniel J. Ford
      Updated coefficients to Wanninkhof et al. (2023) (https://doi.org/10.1016/j.marchem.2022.104178)
      """
      pCO2_Teq = pCO2_SST * np.exp(-0.0413 * dT)
...
#      raw_input("modified...");
   ...
   pCO2_Tym = pCO2_Teq * np.exp(0.0413 * (Tcl_C - Teq_C)) #- \
